...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/07/24 21:51:58 by kkarakur          #+#    #+#             */
/*   Updated: 2023/07/24 21:51:59 by kkarakur         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_format(va_list *arg, char c)
{
	if (c == 'c')
		...
		return (ft_point(va_arg((*arg), unsigned long long int)));
	else if (c == 'u')
		return (ft_unnbr(va_arg((*arg), unsigned int)));
	else if (c == 'x')
		return (ft_hex(va_arg((*arg), unsigned int), 'x'));
	else if (c == 'X')
		return (ft_hex(va_arg((*arg), unsigned int), 'X'));
	else if (c == '%')
		return (write(1, "%%", 1));
